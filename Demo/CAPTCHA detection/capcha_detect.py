...
# Model preparation 
PATH_TO_FROZEN_GRAPH = 'frozen_inference_graph3.pb'
...
# Detection
def main(image, average_distance_error=3):
    with detection_graph.as_default():
        with tf.compat.v1.Session(graph=detection_graph) as sess:
            # Open image
            img = Image.open(image).resize((546,144)) #resize()
            numpy_image=np.array(img)  
            image_np=cv2.cvtColor(numpy_image, cv2.COLOR_RGB2BGR) #convert iamge PIL to cv2 
...
#             cv2.imwrite("Predicted_captcha.jpg", cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB))
            ...
            #write file text detect
            f = open("capcha_detect.txt", "w")
            f.write(captcha_string)
            f.close()
